...
runInstall(){
  ...
  showNotice "(Step 5/7) 是否安装 LiteSpeed"
  ...
  showNotice "(Step 7/7) 使用镜像服务器下载rpms"
  echo "1) 国外源"
  echo "2) 国内镜像源"
  ...
  showNotice "使用Triay Key或序列号激活LiteSpeed"
  echo "1) Triay Key，请将Trial.key放在/root/trial.key中"
  echo "2) 序列号"
  ...
  if [ ! -d "/tmp/LSWS-${envType}" ]; then
    cd /tmp || exit
    if [ ! -f "LSWS-${envType}.zip" ]; then
      if ! curl -L --retry 3 -o "LSWS-${envType}.zip" "https://github.com/Celloey/LSWS/archive/${envType}.zip"
      then
        showError "LSWS-${envType} 下载失败！"
        exit
      fi
    fi
    unzip -q "LSWS-${envType}.zip"
  fi
  ...
  yum -y remove mariadb*
    if [[ "${mysqlV}" = "1" || "${mysqlV}" = "2" || "${mysqlV}" = "3" || "${mysqlV}" = "4" || "${mysqlV}" = "5" ]]; then
      ...
      rpm --import /tmp/LSWS-${envType}/keys/RPM-GPG-KEY-mysql
  ...
  if [ "${phpV}" != '0' ]; then
    ...
    rpm --import /tmp/LSWS-${envType}/keys/RPM-GPG-KEY-remi
  ...
  if [ "${LiteSpeedV}" != '0' ]; then
    ...
    if [ -d "/usr/local/lsws/" ]; then
      mv -bfu /usr/local/lsws/conf/httpd_config.xml /usr/local/lsws/conf/httpd_config.xml.LSWS.bak
      mkdir -p /usr/local/lsws/conf/vhosts/
    fi

    cp -a /tmp/LSWS-${envType}/conf/httpd_config.xml /usr/local/lsws/conf/httpd_config.xml
    cp -a /tmp/LSWS-${envType}/conf/vhosts/example.xml /usr/local/lsws/conf/vhosts/example.xml
    chown lsadm:lsadm /usr/local/lsws/conf/httpd_config.xml
    chown -R lsadm:lsadm /usr/local/lsws/conf/vhosts/

    mkdir -p /home/example/{public_html,logs,ssl,cgi-bin,cache}

    cp -a /tmp/LSWS-${envType}/home/example/public_html/* /home/example/public_html/
    cp -a /tmp/LSWS-${envType}/home/example/ssl/* /home/example/ssl/

    chown -R nobody:nobody /home/example/public_html
    chown -R lsadm:lsadm /home/example/ssl
    ...
    case ${phpV} in
      1)
      sed -i "s@lsphp73@lsphp54@g" /usr/local/lsws/conf/vhosts/example.xml
      ;;
      2)
      sed -i "s@lsphp73@lsphp55@g" /usr/local/lsws/conf/vhosts/example.xml
      ;;
      3)
      sed -i "s@lsphp73@lsphp56@g" /usr/local/lsws/conf/vhosts/example.xml
      ;;
      4)
      sed -i "s@lsphp73@lsphp70@g" /usr/local/lsws/conf/vhosts/example.xml
      ;;
      5)
      sed -i "s@lsphp73@lsphp71@g" /usr/local/lsws/conf/vhosts/example.xml
      ;;
      6)
      sed -i "s@lsphp73@lsphp72@g" /usr/local/lsws/conf/vhosts/example.xml
      ;;
      7)
      sed -i "s@lsphp73@lsphp73@g" /usr/local/lsws/conf/vhosts/example.xml
      ;;
    esac
  ...
  if [[ "${phpV}" != '0' && "${LiteSpeedV}" != '0' ]]; then
    if [ "${dbV}" = "1" ]; then
      cp -a /tmp/LSWS-${envType}/DB/Adminer /home/example/public_html/
      sed -i "s/phpMyAdmin/Adminer/g" /home/example/public_html/index.html
    elif [ "${dbV}" = "2" ]; then
  ...
  if [ "${dbV}" = "2" ]; then
  mkdir -p /home/example/public_html/phpMyAdmin/tmp/
  chmod 0777 /home/example/public_html/phpMyAdmin/tmp/
  cp /home/example/public_html/phpMyAdmin/libraries/config.default.php /home/example/public_html/phpMyAdmin/config.inc.php
  sed -i "s@UploadDir.*@UploadDir'\] = 'upload';@" /home/example/public_html/phpMyAdmin/config.inc.php
  sed -i "s@SaveDir.*@SaveDir'\] = 'save';@" /home/example/public_html/phpMyAdmin/config.inc.php
  sed -i "s@host'\].*@host'\] = '127.0.0.1';@" /home/example/public_html/phpMyAdmin/config.inc.php
  sed -i "s@blowfish_secret.*;@blowfish_secret\'\] = \'$(cat /dev/urandom | head -1 | base64 | head -c 45)\';@" /home/example/public_html/phpMyAdmin/config.inc.php
  fi
  ...
  if [ "${LiteSpeedV}" != '0' ]; then
    ...
    echo "admin:$LSPASSRAND" > /root/defaulthtpasswd
    systemctl restart lsws.service
  fi

  wget -P /root/ https://raw.githubusercontent.com/Celloey/LSWS/master/vhost.sh

  if [[ -f "/usr/sbin/mysqld" || -f "/usr/sbin/php-check" || -f "/usr/local/lsws/bin/httpd" ]]; then
    echo "================================================================"
    echo -e "\\033[42m [LSWS] Install completed. \\033[0m"

    if [ "${LiteSpeedV}" != '0' ]; then
      echo -e "\\033[34m Web Demo Site: \\033[0m http://${ipAddress}"
      echo -e "\\033[34m Web Demo Dir: \\033[0m /home/example/public_html"
      echo -e "\\033[34m LiteSpeed: \\033[0m /usr/local/lsws/"
      echo -e "\\033[34m LiteSpeed WebAdmin Console URL: \\033[0m http://${ipAddress}:7080"
      echo -e "\\033[34m LiteSpeed WebAdmin Console Username: \\033[0m admin"
    ...
    echo -e "\\033[41m [LSWS] Sorry, Install Failed. \\033[0m"
  fi
}
